...
def init_create_folder_database():
	...
	if not os.path.exists("new_gestures"):
		os.mkdir("new_gestures")
	if not os.path.exists("new_gesture_db.db"):
		conn = sqlite3.connect("new_gesture_db.db")
...
def store_images(g_id):
	...
	create_folder("new_gestures/"+str(g_id))
	pic_no = 2400
	...
	while True:
		...
		if len(contours) > 0:
			...
			if cv2.contourArea(contour) > 10000 and frames > 50:
				...
				cv2.imwrite("new_gestures/"+str(g_id)+"/"+str(pic_no)+".jpg", save_img)
